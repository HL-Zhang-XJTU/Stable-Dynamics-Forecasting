...
from model_SGDM import Dataset, SGDM
...
nlf_layer = [output_dim, 50, 50, output_dim]
scale = 128
avgPool_dim = 7
d_model = 64
n_heads = 16
...
for fine_num in range(1):
    ...
    if pre_train is None:
        model = SGDM(dim_in=input_dim, dim_out=output_dim, len_in=win_past, len_out=win_future,
                     nlf_layersize=nlf_layer, scale=scale, avgPool_dim=avgPool_dim,
                     d_model=d_model, n_heads=n_heads,
                     e_layers=enc_layers, d_layers=dec_layers, d_ff=d_ff)
        model = model.cuda()
    else:
        model = torch.load(f"../results/SGDM/{pre_train}/best_model.pth")
    ...
    if not os.path.exists(f"../results/SGDM/{save}"):
        os.mkdir(f"../results/SGDM/{save}")
    else:
        save += "_dump"
        os.mkdir(f"../results/SGDM/{save}")
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if loss < least_loss:
                least_loss = loss
                torch.save(model, f'../results/SGDM/{save}/best_model.pth')
        print("dev loss:", loss)

    np.save(f"../results/SGDM/{save}/train_loss.npy", np.array(train_loss))
    np.save(f"../results/SGDM/{save}/validation_loss.npy", np.array(val_loss))
    np.save(f"../results/SGDM/{save}/learning_rates.npy", np.array(lrs))
    plt.plot(train_loss, label='train loss')
    plt.plot(val_loss, label='validation loss')
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"../results/SGDM/{save}/loss.png", dpi=500)

    plt.figure()
    plt.plot(lrs)
    plt.tight_layout()
    plt.savefig(f"../results/SGDM/{save}/learning_rate.png", dpi=500)

    torch.save(model, f'../results/SGDM/{save}/last_model.pth')
    with open(f"../results/SGDM/{save}/Trained_info.md", 'w') as f:
